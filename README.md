...
- `tests/competency/questions.yaml` + `tests/run_competency.py` — outcome-oriented competency suite: a test-owned registry of the business/technical/multi-hop questions each client ontology must answer, checked deterministically against a throwaway SQLite export (projection-scoped `entities`/`rules`/`projection_resources`/`relationships`/bounded-`path` queries, with drift-, loading-, resolver-read-, and relationship/path scope-isolation regressions).
...
- `docs/coverage.md` — client coverage contract (issue #41): question-family coverage matrix per client, coverage-status vocabulary, and the three maturity dimensions (model representation vs client coverage vs runtime queryability).
...
- `run_competency.py` exercises the competency suite — a test-owned registry (`tests/competency/questions.yaml`) of the business, technical/data-flow, and bounded multi-hop questions each client ontology must answer (the proof behind `docs/coverage.md`). Its projection-scoped query vocabulary is `entities`/`rules`/`projection_resources` plus (issue #41) `relationships` (subject/predicate/object rows, in scope only when the edge's module is in scope **and both endpoints** are in-scope entities) and `path` (a deliberately bounded, deterministic simple-path traversal with explicit `start`/`end` constraints, allowed `predicates`, and `min_hops`/`max_hops` — **not** a general graph query; no GraphRAG, embeddings, model grading, or second store). Loading is **projection/client-directed**: for each question it builds a throwaway SCOPED SQLite export via the shared loader/export path (never the repo's `build/`) from only the named client's manifest, `client.yaml`, the named projection, and the modules that projection references — it never parses another client's files and never parses a module the projection excludes (`export(..., paths=...)` rejects any out-of-root path before parsing). On top of that scoped export it further scopes each query's **results** through the named projection and compares the normalized answer to the expected answer in the registry. Every relationship/path `predicate`, entity `entity_type`, `source_confidence`, `status`, and rule `severity` a question names is validated against the **same controlled vocabulary the schema enforces** (loaded from `schemas/*.schema.json`, with the schema's bounded `x_` predicate escape), and every filter/expected/field-guard operand is type-checked per column (a bool/number against a string column is rejected, and a `require_field_equals`/`require_field_in`/`forbid_field_in` operand on the boolean `public_facing` column must be a real `true`/`false`), so a misspelled predicate, a `subject: false` filter, or a `public_facing: 0` guard is a usage error, not a silently-empty answer that a required question could report as PASS; a stored `public_facing` boolean (SQLite `0/1`) is normalized back to a real `bool` and both row comparison and field-guard evaluation are JSON/type-sensitive so a `false`/`0` type drift is caught rather than silently passed. A `required` question is **gating** and must assert a **non-empty** positive answer; being required does not by itself make a family `covered` (`covered` in `docs/coverage.md` is the subset of required questions whose retrieved resources are evidence-backed — a required question may instead assert a status-awareness safety property over draft resources). A deliberately empty answer must be `required: false` (never cited as `covered`), and a failed optional question is reported honestly rather than absorbed into a blanket "all passed". The `expect` envelope is closed and each expected path chain is validated against its query's hop bounds, allowed predicates, and endpoint constraints, rejecting a repeated-node (cyclic) chain and (once the scoped export exists) an expected endpoint whose `entity_type` contradicts the query; parallel edges collapse to one path (the public representation carries no edge identity). It emits a human report and `--json`, names any failed question with expected-vs-actual diagnostics, exits non-zero on a failed required question, and runs seven regressions: a loading-isolation regression and a synthetic resolver-read isolation regression — both instrumenting the **actual `parse_yaml` calls** — proving no question's scoped load reaches another client or opens an excluded module; a synthetic relationship/path scope-isolation regression proving (on a full export, at the result boundary) that a relationship endpoint in an excluded module is dropped and a bounded path never traverses an out-of-scope node, a disallowed predicate, or beyond its hop bound; a synthetic path-shape regression proving parallel edges dedupe to one path, a back-edge cannot make a simple-path traversal revisit a node, branching yields distinct paths, ordering is deterministic, and an expected endpoint's `entity_type` must match the query's constraint; a drift-isolation regression proving a single controlled semantic change (a metric's status, a projection's membership, a relationship's confidence, or a path edge's confidence) **newly** fails **only** the relevant question, measured against the clean baseline so an already-failing optional question stays non-gating; a reporting-seam regression proving a `public_facing` boolean normalizes to a real `bool` with JSON/type-sensitive comparison and that `--no-drift` represents the drift regression **explicitly as skipped** rather than silently "passed"; and a registry shape-validation regression proving a malformed question (non-string `id`/`client_id`/`projection`, non-boolean `required`, a required question with an empty expected answer, unknown select column, misspelled guard operand, a guard not bound to a selected output column — including an edge-confidence guard on a non-path op or a relationship isolation guard without both endpoints selected, a non-scalar or wrong-typed filter operand, a field-guard operand whose type does not match its column (an int/str on the boolean `public_facing`), a typo'd predicate/entity_type/confidence/status/severity token, a malformed `path` contract, a stray/unknown `expect` key, or an expected chain that contradicts its query — a disallowed predicate, a bad hop count, a mismatched endpoint, or a repeated node) is rejected as a usage error (exit 2) before any answer is trusted. Competency questions are test requirements — not evidence, canonical truth, or authority — and no model, network, API credential, or live client system is required.
- `run_cli.py` exercises the runtime consumer surface (`scripts/ontology_service.py` + `scripts/ontology_cli.py`): the issue #19 CLI acceptance cases (a JMD blocking copy check exits non-zero; a Femme warning exits 0 until `--fail-on warning`; `context --projection femme-events.local-seo` resolves the projection's entities + rules), structured non-zero errors for unknown client/projection, unavailable SQLite, backend drift and malformed args, YAML⇄SQLite backend parity for every operation, projection isolation (a negative leakage case proving an excluded module never appears), planning-only preservation of Femme's draft/`baseline: unknown` metrics, and competency-corpus reuse — every **service-served** (`entities`/`rules`/`projection_resources`) competency question answered through the service equals `run_competency.py`'s own computed answer in both backends, reusing `evaluate_suite`/`load_questions` so no expected value is re-encoded. The relationship/`path` competency ops are test-owned and are **not** served by the read-only service yet; their runtime consumer surface is deferred to the separate query-surface issue, so this test asserts service YAML/SQLite parity only for the served ops and gates the deferred ops through the runner's required-pass check (see `docs/coverage.md`, runtime queryability).
