...
- `run_competency.py` exercises the competency suite — a test-owned registry (`tests/competency/questions.yaml`) of the business/governance questions each client ontology must answer. It builds one throwaway, full canonical SQLite export via the shared loader/export path (never the repo's `build/`) — the export parses every client's YAML — then scopes each query's **results** through the named projection and compares the normalized answer to the expected answer in the registry (isolation is a property of the answer, not of file loading). It emits a human report and `--json`, names any failed question with expected-vs-actual diagnostics, exits non-zero on a failed required question, and runs two regressions: a drift-isolation regression proving a single controlled semantic change fails **only** the relevant question, and a registry shape-validation regression proving a malformed question (unknown select column, misspelled guard operand, wrong-typed expect payload) is rejected as a usage error before any answer is trusted. Competency questions are test requirements — not evidence, canonical truth, or authority — and no model, network, API credential, or live client system is required.
...
`.github/workflows/validate.yml` runs on every push and pull request. It installs Python 3 and Ruby (the validator and exporter shell out to `ruby -e` for YAML parsing), then runs, in order: ontology validation, the SQLite export, `tests/run_fixtures.py`, `tests/run_predicates.py`, `tests/run_export.py`, `tests/run_competency.py`, `tests/run_checks.py`, and `tests/run_evidence.py`. Any validation, export, or check failure fails the build, so malformed data cannot merge — including a competency regression where a schema-valid change silently breaks a consumer answer. A final advisory `check_evidence.py --strict` step re-hashes cited spans and checks source existence: a repo-relative drift or a missing repo-relative source blocks, but external absolute paths unavailable on the runner report `unresolvable_in_environment` / source `unavailable_in_environment` and never fail the build.
