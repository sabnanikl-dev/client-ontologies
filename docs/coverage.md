...
| client coverage | `tests/run_competency.py` — each client answers its business/technical/multi-hop competency questions against the canonical export, with drift-, loading-, resolver-read-, relationship/path scope-isolation, and path-shape (parallel-edge/cycle/branching/order **and expected-endpoint entity_type compatibility**) regressions, plus a registry shape-validation pass that rejects a malformed query/predicate/expect contract as a usage error before any answer is trusted. A `required` (coverage-proof) question must assert a **non-empty** expected answer — an empty required expectation with vacuous guards can never masquerade as coverage; a deliberately empty answer must be `required: false` (a future explicit absence-query DSL stays out of scope). Filter/expected operands are type-checked per column (a bool/number against a string column is rejected), and `severity` is validated against the schema enum |
