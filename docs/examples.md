...
What the nine live questions protect:
...
- **Femme measurement flow** (`femme-events.local-seo`, `relationships`): which outcome metrics `measures` the GBP system, guarded to stay `draft` so a planned data-flow is never shown as verified live instrumentation.
