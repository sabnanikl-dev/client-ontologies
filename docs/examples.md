...
Each query's **results** are scoped **through the named projection** (a row is in scope only if its module is in `includes.modules` or its id is named/`.*`-matched in `includes.entities`/`includes.rules`). The runner builds one shared canonical export — which parses every client's YAML — and then restricts each answer to the question's client and projection, so no other client's rows and no unlisted-and-unreferenced module's rows can appear in an answer (result isolation, not projection-directed file loading). Two deterministic regressions back this: a **drift-isolation** case mutates one point of a copy of the export (a metric's status; a projection's membership) and proves the change fails **only** the relevant question with an expected-vs-actual diagnostic; a **registry shape-validation** case proves a malformed question — an unknown select column, a misspelled guard operand, a wrong-typed expect payload — is rejected as a usage error before any answer is trusted.
