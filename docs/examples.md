...
Goal: guarantee that a schema-valid change cannot silently break the business/governance question a client ontology exists to answer. Structural validation asks *"is this ontology well-formed?"*; the competency suite asks *"can a consumer still get the correct, status-aware answer?"*. The corpus is the proof behind the client coverage contract (`docs/coverage.md`): a family marked `covered` there is credible only because a competency question retrieves the answer here.
...
What the thirteen live questions protect (twelve required, one optional):

- **Femme identity & offering** (`femme-events.agent-context`, `entities`): the verified `brand.identity` and the approved/owner-reviewed `website.service-package` (public offering) — a source-backed business-identity answer (both are evidence-backed recorded resources).
- **Femme metrics, planning-only** (`femme-events.local-seo`): the three GBP outcome metrics stay `status: draft` with `baseline: unknown`, so a reporting consumer cannot present them as recorded results. This is a **status-awareness safety check**, not coverage proof — the metric-definition family is `deferred / trigger-gated` in `docs/coverage.md` because the draft entities carry no evidence.
...
- **Femme GBP grounding** (`femme-events.local-seo`, `relationships`): the Google Business Profile `uses` an owner-reviewed business fact that `targets` a service area — a relationship-backed business answer, guarded to stay owner-reviewed (never draft guesswork).
- **Femme website data-flow** (`femme-events.website-build`, `relationships`): the **verified**, source-backed integration surface — Sanity CMS content `renders_in` the site, and the site `targets` the inquiry form — guarded so every returned edge is `verified` (Femme's source-backed technical/data-flow answer).
- **Femme content path** (`femme-events.website-build`, `path`): the verified two-hop flow `cms-content --renders_in--> site --contains--> service-package`, guarded so every edge is `verified`.
- **Femme measurement flow, planning-only** (`femme-events.local-seo`, `relationships`, **optional**): the three metric→GBP `measures` edges are `draft` with no evidence; this optional status-awareness check proves they stay `draft` so a *planned* data-flow is never shown as verified live instrumentation. It is explicitly **not** a source-backed answer.
- **JMD identity & differentiator** (`jmd-menswear.website-build`, `entities`): the verified `brand.identity` and the durable one-of-a-kind `brand.differentiator` — a source-backed business-identity answer, guarded to stay `active`/`verified`.
...
- **JMD owner roles** (`jmd-menswear.website-build`, `entities`): the verified `operations.owner-roles` records the Lucky/Danny responsibility split — a source-backed people/roles answer, guarded to stay `active`/`verified`.
- **JMD governance edge** (`jmd-menswear.website-build`, `relationships`): the approval boundary `governs` the public showroom site — a verified operating-constraint relationship.
...
- **JMD inventory pipeline** (`jmd-menswear.inventory-workflow`, `path`, **required multi-hop**): the three-hop `drive-folder --contains--> image --creates_or_updates--> sanity-asset --renders_in--> showroom-item` data-flow, guarded to stay `draft` so the proposed MVP pipeline cannot pose as shipped architecture.

Loading is **projection/client-directed**: for each question the runner reads only the named client's manifest, `client.yaml`, the named projection, and the module files that projection references (`includes.modules`; when a reference points at a module outside `includes.modules` the scope widens to the full single-client module set rather than scanning-and-excluding other modules) — it never parses another client's files and never parses a module the projection excludes, so a Femme question reads only Femme files and a projection that excludes a module never reads it. On top of that scoped export each query's **results** are further scoped **through the named projection** (a row is in scope only if its module is in `includes.modules` or its id is named/`.*`-matched in `includes.entities`/`includes.rules`; a **relationship** surfaces only when its module is in scope **and both endpoints** are in-scope entities; a **path** traverses only scoped edges and never crosses a projection or client boundary), so no other client's rows and no unlisted-and-unreferenced module's rows can appear in an answer. Seven deterministic regressions back this: a **loading-isolation** case and a synthetic **resolver-read isolation** case — both instrumenting the runner's **actual `parse_yaml` calls** (not just the returned path list) — prove no cross-client file and no excluded-module file is opened, even transiently during reference resolution; a synthetic **relationship/path scope-isolation** case (evaluated on a *full* export, so it targets result-level scoping) proves a relationship whose object lives in an excluded module is dropped and a bounded path never traverses an out-of-scope node, a disallowed predicate, or beyond its hop bound; a synthetic **path-shape** case proves parallel edges (identical endpoints/predicate/confidence, no edge identity in the public representation) collapse to **one** path, a back-edge cannot make a simple-path traversal revisit a node, branching yields distinct paths, ordering is deterministic, and an expected endpoint whose actual `entity_type` contradicts the query's `start`/`end` constraint is rejected before evaluation; a **drift-isolation** case mutates one point of a copy of a scoped export (a metric's status; a projection's membership; a relationship's confidence; a path edge's confidence) and proves the change **newly** fails **only** the relevant question with an expected-vs-actual diagnostic — measured against the clean baseline, so an already-failing (non-gating) optional question never turns a drift case red; a **reporting-seam** case proves the runner's answer/report semantics are type- and skip-honest — a `public_facing` boolean stored as SQLite `0/1` is normalized back to a real `bool` and comparison is JSON/type-sensitive (`false` ≠ `0`) for both row comparison **and** field-guard evaluation (a boolean row never loosely satisfies an int/str guard operand), and `--no-drift` represents the drift regression **explicitly as skipped** (not silently "passed"), with the summary saying so rather than claiming drift held; and a **registry shape-validation** case proves a malformed question — a non-string `id`/`client_id`/`projection`, a missing or non-string human-readable `question`/`rationale`, an unknown question-level key (a misspelled `gaurds:` or any other stray key, except a deliberate `x_` extension), a non-boolean `required`, a **required question with an empty expected answer** (a required question is gating and must be a non-empty positive assertion; a deliberately empty answer must be `required: false`), an unknown select column, a misspelled guard operand, a guard not bound to a selected output column (including an edge-confidence guard on a non-path op, or a relationship isolation guard without both endpoints selected), a non-scalar filter operand **or one whose scalar type cannot match its column** (a bool/number against a string column such as `subject`), a **field-guard operand whose type does not match the selected column** (an int/str on the boolean `public_facing` column, which would otherwise pass loosely through Python's `False == 0` at evaluation), a **typo'd `predicate`/`entity_type`/`source_confidence`/`status`/`severity` token** (validated against the same controlled vocabulary the schema enforces, with the bounded `x_` predicate escape), a malformed `path` contract (missing/mistyped `start`/`end`/`predicates`, `min_hops` > `max_hops`, or `max_hops` over the bounded cap), a **stray/unknown `expect` key** (the envelope is closed), or an **expected path chain that contradicts its query** (a disallowed predicate, an out-of-bounds hop count, a mismatched `start`/`end` endpoint, a **repeated node** the simple-path traversal can never return, or a bad confidence token) — is rejected as a usage error (exit 2) before any answer is trusted. A failed **optional** question never gates the exit code, but the summary reports it honestly (`N required passed; K optional FAILED`) instead of a blanket "all passed". `export(..., paths=...)` also rejects any path outside `root` before parsing and always closes its SQLite connection.

### Adding a competency question

1. Pick the client and the **projection** that a real consumer would load for the job.
2. Add an entry to `tests/competency/questions.yaml` with a stable `id`, `client_id`, `projection`, human-readable `question`, `rationale` (the consumer job it protects), a `query`, the `expect`ed answer, and `guards` for the status/safety/isolation boundary. The bounded, deterministic query vocabulary is:
   - `entities` / `rules` — row query with `filters` (column → scalar or list) and `select` (columns or `fields.<name>`); `expect.rows`.
   - `projection_resources` — the projection's declared modules/entities/rules; `expect.resources`.
   - `relationships` — subject/predicate/object edge rows. `filters`/`select` over `subject`/`predicate`/`object`/`source_confidence`/`module_id`; `expect.rows`. An edge is in scope only when its module is in scope **and both endpoints are in-scope entities**. A `forbid_id_prefix` isolation guard here must select **both** `subject` and `object`.
   - `path` — a **deliberately bounded** multi-hop traversal (not a general graph query): name `start` and `end` node constraints (`id` / `id_prefix` / `entity_type`), an allowed-`predicates` list, and `min_hops`/`max_hops` (1 ≤ min ≤ max ≤ 6). `expect.paths` is a list of `{chain: [node, predicate, …, node], confidences: [per-edge …]}`.
   - Guards: `require_status`/`forbid_status` (row `status`), `require_field_equals`, `require_field_in`/`forbid_field_in` (any selected field — e.g. relationship `source_confidence`), `forbid_id_prefix` (row ids / relationship endpoints / path nodes), and `require_edge_confidence_in`/`forbid_edge_confidence_in` (**path only** — prove a draft data-flow is not asserted as verified current architecture). Every guard must bind to the query's output shape or the registry is rejected, and a field guard's operand is container- and type-checked against the selected column: `require_field_equals` takes exactly one scalar (a list/mapping/null is a usage error) and `require_field_in`/`forbid_field_in` take a non-empty list of scalars (**not** hard-coded strings, so a boolean membership operand such as `[false]` is expressible), then every scalar must match the column type — a real `true`/`false` for the boolean `public_facing`, a string for a string column — so a `public_facing: 0` guard, an int/string membership operand on `public_facing`, or an equality operand wrapped in a list can never pass loosely.
3. Keep expected values **only** in the registry — never copy them into runtime/service code. Run `python3 tests/run_competency.py`. The relationship/path ops are **test-owned**: their read-only runtime consumer surface is deferred to the separate query-surface issue (see `docs/coverage.md`, runtime queryability), so `tests/run_cli.py` proves service parity for the served ops and gates the newer ops through the runner's required-pass check.
